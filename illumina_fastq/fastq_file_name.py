...
class FastqFile:
    ...
    def _setReadNumber(self):
        ...
        if readNum not in self.READ_NUMS:
            raise UnknownReadNumberException(
                "Unkwown read number '{readNum}' in FASTQ file name '{name}'. Only the following read numbers are recognized: '{allowed}'.".format(
                    readNum=readNum, name=self.fqf, allowed=self.READ_NUMS))
        self.read = readNum
