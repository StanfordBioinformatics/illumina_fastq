...
parser.add_argument("--outdir",
    help="The pre-existing directory to output the FASTQ files containing the extracted barcodes. Defaults to the current working directory.")
parser.add_argument(
    "--outfile-prefix",
    required=True,
    help="The file prefix of each output FASTQ file for a given barcode. The barcode name will be appended to this prefix, as well as the read number. For example, setting the outfile prefix to 'output' would result in 'output_${barcode}_R1.fastq' and 'output_${barcode}_R2.fastq'.")
parser.add_argument(
    "-b",
    "--barcodes",
    nargs="+",
    help="One or more barcodes to extract from the input FASTQ file(s).")
